...
/* 	$(window).load(function() {
				...
				//else if($scope.terminalData.type == 'FRONT' && isBackOrder(doc._id) && angular.isArray(doc.data.shipping_lines) && doc.data.shipping_lines.length && doc.data.shipping_lines[0].method_id == 'local_pickup' && !doc.data.payment_details.paid && (doc.data.status=='on-hold'||doc.data.status=='kitchen-ready')){
					...
					for(var i=0;i<temp.data.length;i++){
						if(!temp.data[i].status){
							doMutexOperation(print_mutex,{'type':temp.data[i].type,'order':temp.data[i].order.data,'cart':[],'void_items':temp.data[i].order.void_items},false);
	...
	$scope.updatePaidStatus = function(order,online_count){
	...
	var createOrder = function(status){
		var now_date = new Date().toISOString();
		var fee_lines = [];
	...
	var doMutexOperation = function(mutex,action,show_dialog=true) {
   ...
   $scope.complete = function(status) {
		...
		if(status != 'pending' && status != 'cancelled'){
			...
			if($scope.startup.action == 'TABLEVIEW'){
				...
				new_order = recreate_order($scope.cartItems,status);
			}else{
