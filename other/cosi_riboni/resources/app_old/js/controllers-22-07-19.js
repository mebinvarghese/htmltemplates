...
/* 	$(window).load(function() {
	...
	$scope.closeModal = function(index,manual=false){
		if(index == 'SHOW_OPTIONS'){
			...
			if(!$scope.quickCustomerEnable &($scope.startup.action == "SHIPPING") && !$scope.cartItems.length){
				checkEndAction(true);
			}
		}else if(index == 'CUSTOMER_EDIT'){
...
//------------------start code for deposit module------------------------------------------ //
 ...
 $scope.depositfinish=function(){  //saving deposit to pouch db 
	...
	$pouchDB.upsert($scope.documentname,function(doc){
		...
		$scope.depositpoup('Message','Successeully  Updated');
	}).catch(function(error){
		console.log(error);
	});
 }
...
$scope.redeemDeposite=function(){ //update deposit details to back end
...
var setData =true;
		...
		$pouchDB.bulkFetch(onlineconfig1).then(function(docsdeposit){
			angular.forEach(docsdeposit.rows,function(row){
				if(row.doc.data.Amount !=0 && total&& setData){
					if(parseFloat(row.doc.data.Amount)>total)
						setData=false;
					if(total>=parseFloat(row.doc.data.Amount)){
						$pouchDB.upsert(row.doc._id,function(data){
							data.data.Amount =0;
							return data;
						}).then(function(resp){
							total-=parseFloat(row.doc.data.Amount);
					...
					}else if(parseFloat(row.doc.data.Amount)>total){
						$pouchDB.upsert(row.doc._id,function(data){
			...
			var discount =(total==0)?$scope.grand.Total:($scope.grand.Total-total);
			//$scope.depositInfo[0].data.Amount -=$scope.grand.Total;
			$scope.amount_key = (total==0)?'0.00':total;
			$scope.grand.Discount+=discount;
			$scope.showCheckoutMsg('Message','Successeully  Updated');
		}).catch(function(error){
			console.log(error);
		});
}
