...
.controller('AppCtrl', function($scope, dataService, $localstorage, $timeout, $ionicLoading, $ionicModal, $filter, $state, $rootScope,$ionicPlatform, $ionicHistory,$window,$ionicScrollDelegate,
	...
	$scope.pos_db_name= AIV_CONSTANTS.REMOTE_DB;
	...
	$scope.cardSchemeName="";
	$scope.primaryAccountNumber="";
	$scope.logged_true=true;
	$scope.user_logout=false;
	$scope.formCustomer_find = {
		address_1:"",
		address_2:""
	};

    /*var shopname="Captur Pos Demo";
    var shoponlinenode=firebase.database().ref('Shops/'+shopname);

    var myConnectionsRef = firebase.database().ref('Shops/'+shopname+'/connections');
    var currentstatus = firebase.database().ref('Shops/'+shopname+'/status');
    var lastOnlineRef = firebase.database().ref('Shops/'+shopname+'/lastOnline');
	...
	var connectedRef = firebase.database().ref(".info/connected");
		connectedRef.on("value", function(snap) {
		  ...
		  if (snap.val() === true) {
		  	...
		  	$scope.posisactive=true;
		  	myConnectionsRef.onDisconnect().set(false);
		  	currentstatus.onDisconnect().set('Offline');
		  	lastOnlineRef.onDisconnect().set(firebase.database.ServerValue.TIMESTAMP);
		  	myConnectionsRef.set(true);
		  	currentstatus.set('online');
		  	shoponlinenode.once('value',gotUserData);
	...
	$scope.OnlineOrders = function(action){
		...
		$scope.changeonlineaction(action);
		$localstorage.setObject(AIV_CONSTANTS.LS_PREFIX+'onlineOrder',$scope.onlineOrder);
  ...
  $scope.connectionalive="";
	Offline.on('confirmed-down', function () {
		$scope.connectionalive="offline";
		console.log($scope.connectionalive);
		$scope.pos_status = { 'online':false,'title':'offline','class':'assertive blink','icon':'ion-more'};
...
/* 	$(window).load(function() {
	...
	$scope.processProducts = function()
	{
		angular.forEach($scope.products, function(item) {
			if(item.meta.wc_productdata_options[0].hasOwnProperty('prdt_configs'))
			{
		...
		switch(doc._id){
			...
			case 'getShopInfo':
				...
				if(returnData.extra_info == "1"){
					...
					$rootScope.aiv_info.enableDevMode = returnData.enableDevMode;
					$rootScope.aiv_info.base_url = returnData.base_url;
					$rootScope.aiv_info.loyalty_url = returnData.loyalty_url;
					$rootScope.aiv_info.multi_back = returnData.Mutli_window_background;
					$rootScope.aiv_info.multi_window_image = returnData.multi_window_image;
					$rootScope.aiv_info.multi_window_video = returnData.multi_window_video;
					var multi_init_data ={
						"back":$rootScope.aiv_info.multi_back,
						"image":$rootScope.aiv_info.multi_window_image,
						"video":$rootScope.aiv_info.multi_window_video
					};
					DataTOSecondWindow("init",multi_init_data)
				...
				$localstorage.setObject('BASE_URL',$rootScope.aiv_info.base_url);
				$localstorage.setObject('LOYALTY_URL',$rootScope.aiv_info.loyalty_url);
			...
			case 'getDiscounts':
				...
				var deal_offer = getDealInfo('B17_99');
				if(deal_offer && deal_offer.length){
					$scope.deal_category = deal_offer[0].deal_products;
				}
				break;
	...
	$scope.aiv_settings = {
		selected : 'TERMINAL',
		types:[
			{id:'TERMINAL',name:'Terminal',checked:true},
	...
	$scope.aiv_reports = {
		selected : 'XREPORT',
		types:[
				{id:'XREPORT',name:'X Reports',checked:true},
	...
	$scope.calculate_cart_tax = function(end_discount = 0){
		...
		for(var i=0;i<$scope.cartItems.length;i++){
			...
			if(product.taxable || !product.taxable){
				if(product.tax_class == null || product.tax_class == ""){
					product.tax_class = "standard";
				}
				var tax_classObj = $filter('filter')($scope.taxes,{class:product.tax_class});
	...
	var checkEndAction = function(trash){
		...
		if($scope.CID_ph_num){
			...
			if($scope.isopened_paymentscreen)
			{
				$scope.closeModal('OPEN_PAYMENT_SCREEN');
			}
	...
	$scope.openModal = function(index){
		...
		else if(index =='OPEN_PAYMENT_SCREEN'){
			
			$scope.open_payment_screen_modal.show();
			$scope.isopened_paymentscreen=true;
		}
	}
	...
	$scope.closeModal = function(index,manual=false){
		if(index == 'SHOW_OPTIONS'){
			...
			if(!$scope.quickCustomerDisable && ($scope.startup.action == "SHIPPING") && !$scope.cartItems.length){
				checkEndAction(true);
			}
		}else if(index == 'VIEW_ORDER'){
			...
			$scope.open_payment_screen_modal.remove();
			$scope.isopened_paymentscreen=false;
		}
		
	}
	$scope.close_payment_screen=function(close_modal)
	{
		 $scope.closeModal(close_modal);
	...
	$scope.openOrderModal = function(){
		...
		$scope.temp_drivers.unshift({id:-1,name:'Driver',orders:[]});
	}
	...
	$scope.checkBagCharges = function(create){
		...
		if($scope.split_payment.paid){
			...
			var total = $scope.grand.Total + $scope.grand.Shipping - $scope.grand.Discount + $scope.payment_fee + $scope.bagCharges.selected+$scope.grand.Tax;
			if($scope.startup.action=='TABLEVIEW'){
	...
	$scope.saveSettings = function(){
		...
		if($scope.settings_data.terminal.name == ''){
			...
			else if($scope.pos_db_name==undefined || $scope.pos_db_name == ''){
				$scope.settings_error.err = "Please enter valid database name";
				return;	
			}
		...
		if(AIV_CONSTANTS.REMOTE_DB !=$scope.pos_db_name){
			$localstorage.setObject(AIV_CONSTANTS.REMOTE_DB,$scope.pos_db_name);
			restart_required++;
			AIV_CONSTANTS.REMOTE_DB =$scope.pos_db_name;
		}
	...
	$scope.updatePaidStatus = function(order,online_count,dataGet =false){
	...
	var paymentData={description: "Pay with your credit card",
						enable: true,
						fee: "0",
						id: "card",
						method_title: "Card",
    ...
    $scope.amountcard="";
	$scope.payWithcard=function(amount,splitcondi){
		$scope.amountcard=amount;
		$scope.splitcondi=false;
		$scope.savePayment(paymentData);
		if(splitcondi)
		{
			$scope.splitcondi=true;
		}
	}
	$scope.PaymentMethods = $localstorage.getArray(AIV_CONSTANTS.LS_PREFIX+'PaymentMethods');
	$scope.savePayment = function(item,open_pay=false) {
		...
		if(item.id == 'card'){
			$scope.disable_finish=false;
			$scope.isActive_tab_extmach =false;
			$scope.externalmachine_active=true;
			$scope.disable_card_terminal=true;
			$scope.transactionresultmessage="";
	...
	{ 
	    $scope.closeModal('PAYMENT');
		$ionicModal.fromTemplateUrl('openpaymentscreen-modal.html', {
				id:'OPEN_PAYMENT_SCREEN',
				backdropClickToClose: false,
				scope: $scope,
				animation: 'none'
			}).then(function(modal) {
				$scope.open_payment_screen_modal = modal;
				$scope.openModal('OPEN_PAYMENT_SCREEN');
	...
	var createOrder = function(status,split=false,finalBill=false,card=true,cash=false){
		...
		fee_lines.push({
			   ...
			   "title": "CGST",
			   "tax_class": "",
			   "total": $filter('number')($scope.grand.Tax/2,2),
			   "total_tax": $filter('number')($scope.grand.Tax/2,2)
			});
			fee_lines.push({
			   "id":"",
			   "title": "SGST",
		...
		if($scope.payment_fee){
			fee_lines.push({
			   ...
			   "total_tax": $filter('number')($scope.grand.Tax,2)
			})
		}
		...
		if($scope.formCheckout.pending_data.order_no != ''){//Already saved order
			...
			var new_order = {
			   ...
			   "data":{
				   ...
				   "currency": "INR",
				   "total": ($scope.grand.Total + $scope.grand.Shipping - $scope.grand.Discount + $scope.grand.Tax +$scope.payment_fee + $scope.bagCharges.selected).toFixed(2),
				   "subtotal": $scope.grand.Total.toFixed(2),
				   "total_line_items_quantity": $scope.grand.Quantity,
				   "total_tax":$scope.grand.Tax.toFixed(2),
				   "total_shipping": $scope.grand.Shipping,
				   "cart_tax": $scope.grand.Tax,
				   "shipping_tax": "0.00",
	...
	$scope.printRawReceipt = function(action,show_dialog=true,open_cashdrawer=false,reprintKOT){
		...
		var get_order_data = function(){
			...
			if(order.payment_details.paymentsense==true)
			{
				rec_footercard='Card Type: '+$scope.cardSchemeName;
				rec_footercardaccnt='Card No: '+$scope.primaryAccountNumber;
			...
			.then(function(mutex){
				...
				getDailyOrderNo(true).then(function(ordernum_data){
					action.order.order_meta.daily_order_no = $scope.dailyOrderNo.prefix+$scope.dailyOrderNo.num;
					 $pouchDB.upsert(id,function(doc){
					doc.order_meta.daily_order_no = $scope.dailyOrderNo.prefix+$scope.dailyOrderNo.num;
					return doc;
					}).then(function(result) {
						console.log(result);
					}).catch(function(error){
	...
	var create_print_template = function(printer_settings,type,print_margin){
		...
		if(type == 'TABLE_KOT'){
			...
			if(rec_footercard){
				printData.push({ type: 'raw', format:'plain', data: gen_print_command(printer_settings.cmd_bon) });
				source = "{{center:rec_footercard}}";
				gen_print_template(template,printData,source,printer_settings,1);
				printData.push({ type: 'raw', format:'plain', data: gen_print_command(printer_settings.cmd_boff) });
			}
			if(rec_footercardaccnt){
				printData.push({ type: 'raw', format:'plain', data: gen_print_command(printer_settings.cmd_bon) });
				source = "{{center:rec_footercardaccnt}}";
				gen_print_template(template,printData,source,printer_settings,1);
				printData.push({ type: 'raw', format:'plain', data: gen_print_command(printer_settings.cmd_boff) });
			}
			...
			if(rec_footer4){
				source = "{{var:rec_footer4}}";
				gen_print_template(template,printData,source,printer_settings,1);
			}
			
			source = ".hline {{center:rec_header2}} .hline";
			gen_print_template(template,printData,source,printer_settings,1);
								...
								arr = [item.quantity,prdt_name,$filter('number')(item.subtotal,2)];
									for(var d=0;d<prdtDepts.length;d++){
										...
										if(checkPrdtGrp(item.group)){
											output[prdt_dept].push(['','INNER_DASH','']);
	...
	$scope.printRawReport = function(report){
		...
		/* rec_map6 = [
			["Hot     ", $scope.aiv_report.by_product_type.hot.count, $scope.aiv_report.by_product_type.hot.amount.toFixed(2)],
			["Cold    ", $scope.aiv_report.by_product_type.cold.count, $scope.aiv_report.by_product_type.cold.amount.toFixed(2)]
		] */;
		...
		//rec_footer6 = [["Total x "+$scope.aiv_report.by_product_type.total.count,"", $scope.aiv_report.by_product_type.total.amount.toFixed(2)]];
		...
		source+=" {{var:rec_header2}} .hline \
				  {{map:rec_map2}} .hline \
				  {{map:rec_footer2}} .newline\
				  {{var:rec_header3}} .hline \
				  {{map:rec_map3}} .hline \
				  {{map:rec_footer3}} \
				  {{map:rec_footercard}} \
				  {{map:rec_footercardaccnt}} \
	...
	var calculateReport = function(orders){
		...
		for(var i=0;i<orders.length;i++){
			...
			if(!$scope.report_filters.pos || (angular.isDefined(order.order_meta.processed_by) && $scope.report_filters.pos == order.order_meta.processed_by)){
				...
				doc.by_sales.total_tax+=parseFloat(order.total_tax);
   ...
   $scope.complete = function(status,split=false,card=true,cash=false) {
	   ...
	   else if($scope.splitcondi)
	   {
		...
		if(!$scope.online_processing){//Not a Pending online order
			if($scope.terminalData.type != 'FRONT' && $scope.startup.action!='TABLEVIEW' && $scope.ship_select.id == null){
				...
				$scope.pos_action.done_clicked = false;
				return;
			}else if(angular.isArray($scope.formCheckout.shipping_lines) && $scope.formCheckout.shipping_lines.length && $scope.formCheckout.shipping_lines[0].method_id != '' && $scope.formCheckout.customer_id == 'guest'){
		...
		if(status != 'pending' && status != 'cancelled'){
			if(!validate_payment()){
				if(!$scope.showSplitPay.show){
					$scope.pos_action.done_clicked = false;
					$scope.disable_card_terminal=false;
		...
		if(status != 'pending' && status != 'cancelled'){
			$scope.closeModal('PAYMENT');
			$scope.splitcondi=false;
			if(new_order.data.payment_details.method_id=='cod'&&!new_order.data.payment_details.split)/*card_add*/
			{
				new_order.data.payment_details.cash=new_order.data.total;
			}
			else if(new_order.data.payment_details.method_id=='cod'&&new_order.data.payment_details.split)
			{
				new_order.data.payment_details.cash=$scope.showSplitPay.amount;
			}
			if(new_order.data.payment_details.method_id=='card'&&!new_order.data.payment_details.split)/*card_add*/
			{
				new_order.data.payment_details.card=new_order.data.total;
			}
			else if(new_order.data.payment_details.method_id=='card'&&new_order.data.payment_details.split)/*card_add*/
			{
				new_order.data.payment_details.card=$scope.showSplitPay.amount;
			}
			if($scope.paymentsense==true)
			{
				$scope.with_cardonly=true;
			 
			   var newNum = "paymentsense";
               var newVal = true;
               new_order.data.payment_details[newNum] = newVal;

			}
			else
			{
				$scope.with_cardonly=false;
				var newNum = "paymentsense";
                var newVal = false;
                new_order.data.payment_details[newNum] = newVal;
			}
		...
		$pouchDB.get($scope.formCheckout.pending_data.order_id).then(function(order){
			...
			if($scope.payment_fee){
				new_order.data.fee_lines.push({
				   ...
				   "tax_class": "",
				   "total": $filter('number')($scope.payment_fee,2),
				   "total_tax":$filter('number')($scope.grand.Tax,2)/*GST*/
				})
			}
	...
	$scope.aivTableInfo =[];
		for(var i=0;i<10;i++){
			var tableInfo =[];
			for(var j=0;j<10;j++){
				var tableDoc = {"number":"","shape":"","status":"","selected":"","lock":false,"index":-1};
				tableInfo.push(tableDoc);
			}
		$scope.aivTableInfo.push(tableInfo);
		}
		for(var k=0;k<$scope.aiv_tables.length;k++){
			var tableDo_1 = {"number":$scope.aiv_tables[k].number,"shape":$scope.aiv_tables[k].shape,"status":$scope.aiv_tables[k].status,"selected":true,"lock":$scope.aiv_tables[k].lock,"index":k};
	...
	$scope.onKeyPressed = function(data,type) {
		switch(type){
			...
			case 'CUSTOMER_EDIT_FNAME':
				if (data == '⇐' || data == $scope.aiv_bksp_key) {
			...
			case 'SMS_CONTENT':
				...
				break;
			case 'SMS_SEARCH':
				if (data == $scope.aiv_bksp_key || data=='⇐') {
...
/* 			$pouchDB.find(find_index,find_selector,find_params).then(function (return_data) {
	...
	$scope.setLocalIp = function(type){
		...
		var ip = '',db = '';
		if(type == "PRIMARY"){
			ip = $scope.settings_data.localdb_ip.split(";")[0];
			db = $scope.settings_data.localdb_ip.split(";")[1];
		}else{
		...
		var db_url = createDBUrl(ip,'pos_'+db);
		dataService.checkUrl(db_url).then(function(res) {
			...
			if(type == "PRIMARY"){
				$scope.localdb_ip = ip;
				$localstorage.setObject(AIV_CONSTANTS.LS_PREFIX+'localdb_ip',$scope.localdb_ip);
				$localstorage.setObject('app_db_name_1',db);
				reloadApp();
			}else{
	...
	$scope.startAction = function(action,method = null){
		...
		switch(action){
			...
			case 'CALL_ORDER':
			case 'REPORTS':
			      $scope.aiv_reports.selected='XREPORT';
			case 'CUSTOMERS':
	...
	var getDataOffline = function(){
		...
		var docs = [
			...
			'getSettings','getUKPostcodes','getTaxclasses','getTaxes',
	...
	if($scope.localdb_ip!='' && (AIV_CONSTANTS.BASE_URL =="" || AIV_CONSTANTS.LOYALTY_URL =="")){
				$timeout(function(){
					reloadApp();
				},10000);
	 }
	 else if($scope.localdb_ip!='' &&(angular.equals(AIV_CONSTANTS.BASE_URL, {}) || angular.equals(AIV_CONSTANTS.LOYALTY_URL, {}))){
			$timeout(function(){
					reloadApp();
				},10000);
	 }
	var DataTOSecondWindow = function(item,data){
	//	var Data = {item:data};
		var Data={
			"title":item,
			"value":data
...
$scope.paymentsense=false;

	//get all termoinals
$scope.all_terminals=function() {
	$scope.terminaldetisl="";
	$scope.isActive_tab = true;
	$scope.isActive_tab_extmach =false;
	$scope.ispaymentsenseactive=false;
	$scope.externalmachine_active=true;
	$scope.response_message = false;
	$scope.time_out_response=false;
	$scope.loading_error = false;
    
    if($scope.transactionresultmessage)
    {
    	 $scope.response_message = true;
    }
	 $scope.loading = true;
	 $scope.httpRequest = dataService.getpos_terminals();

             $scope.httpRequest.then(function(returnData) {
             	 
             $scope.terminaldetislength=returnData;

             	if( $scope.terminaldetislength.length>0)
             	{
             			$scope.loading = false;
             			$scope.terminaldetisl=returnData;
             	}
             	else
             	{   $scope.loading = false;
             		$scope.errorterminal="No terminals available";
             	}
		    ...
		    $scope.error_call=function() {
				     if(err.error=="timeout")
				     {
				     	$scope.loading = false;
				     	$scope.time_out_response=true;
				     	$scope.timeouterror=err.error+" "+err.message;
				     }

				     else if($scope.connectionalive!="online")
				     {
				     	$scope.loading = false;
				     	$scope.remove_gettrans= true;
				     	$scope.time_out_response=true;
				     	$scope.timeouterror="No Internet Connection";
				     }
				     else if(err=="Invalid authorization credentials provided.")
				     {  
				     	$scope.loading = false;
				     	$scope.remove_gettrans= true;
				     	$scope.time_out_response=true;
				     	$scope.timeouterror='Invalid authorization credentials provided. Please check your URL, OR API KEY';
				     }
...
//external machine transaction

$scope.exttransaction=function() {
	$scope.terminaldetisl="";
	$scope.isActive_tab = false;
	$scope.isActive_tab_extmach =true;
	$scope.ispaymentsenseactive=true;
	$scope.externalmachine_active=false;
	$scope.payment_select.id='card';
}

//get transaction terminal
$scope.gettransaction = function(tid) {
	$scope.loading_error = false;
	$scope.IsVisible = false;
	$scope.successfull_message=false;
	$scope.selectedterminal=tid;
	$scope.response_message = false;
	$scope.enable_cancel = false;
	$scope.time_out_response=false;
	$scope.httpRequest = dataService.getterminalid(tid);
	$scope.httpRequest.then(function(returnData) {
	  	 $scope.IsVisible = $scope.IsVisible ? false : true;
	  	 $scope.visibleterminal=returnData.tid;
	 }).catch(function(err){
    ...
    //postterminaltransaction
    $scope.posttransaction = function(tid) {
    	
    	if($scope.amountcard=="")
    	{	
    		$scope.amountcard= parseFloat($scope.payment_select.to_pay).toFixed(2);
    	}
    	 $scope.sign_verify=false
    	 $scope.inactive_start=true;
    	 $scope.loading_error = true;
         $scope.inactive_terminal = true;
         $scope.successfull_message=true;
         $scope.loading_error=false;
         $scope.time_out_response=false;
         $scope.notificationresult="";
         $scope.transactionresultmessage="";
    	 $scope.httpRequest = dataService.posttransactionrequest(tid,$scope.amountcard);
    	 $scope.httpRequest.then(function(returnData) {
    	 	        $scope.location=returnData.location;
             		 $scope.request_id=returnData.requestId;
             		 $scope.terminal_id=tid;
             		 if($scope.request_id!="")
             		 {  
             		 	$scope.enable_cancel = true;

             		 	
             		 	var storeTimeInterval = setInterval(function(){
                               
	                           $scope.httpRequest = dataService.gettransactionresult(tid,$scope.request_id);
	             		 	   $scope.httpRequest.then(function(responsedata) {
	             		 	   	console.log(responsedata);
	             		 	   		$scope.notificationresult=responsedata.notifications[0];
	             		 	   		$scope.loading_error = false;
	             		 	   		$scope.result_loader = true;
	             		 	   		if($scope.notificationresult=="TRANSACTION_FINISHED")
           								{    $scope.usermessage=responsedata.userMessage;
							                 $scope.inactive_terminal = false; 
							                 $scope.transactionresult=responsedata.transactionResult;	
							               if($scope.transactionresult=='SUCCESSFUL')
							               {  
							               	  $scope.inactive_start=false;
							               	  $scope.response_message = true;
							               	  $scope.IsVisible =  false;
							               	  $scope.notificationresult="";
							               	  $scope.result_loader = false;
							               	  $scope.transactionresultmessage="Transaction finished and successful";
							               	  $scope.amountcard="";
							               	  $scope.cardSchemeName=responsedata.cardSchemeName;
							               	  $scope.primaryAccountNumber=responsedata.primaryAccountNumber;
							               	  $scope.sign_verify=false;
							               	  $scope.paymentsense=true;

							               }
							               else if($scope.transactionresult=='TIMED_OUT'){
							               	  $scope.amountcard="";
							               	  $scope.inactive_start=false;
							                  $scope.response_message = false;
							                  $scope.enable_cancel = false;
							                  $scope.result_loader = false;
							                  $scope.notificationresult="";
							                  $scope.transactionresultmessage="Timed out the transaction";
							                  $scope.sign_verify=false;
							                  ...
							                  $scope.result_loader = false;
							                  $scope.notificationresult="";
							                  $scope.transactionresultmessage=$scope.transactionresult+' The Transaction';
							                  $scope.sign_verify=false;
                         				...
                         				else if($scope.notificationresult=="SIGNATURE_VERIFICATION"){
                         					 $scope.sign_verify=true;
								    ...
								    $scope.error_call=function() {
										     if(err.error=="timeout")
										     {
										     	...
										     	$scope.time_out_response=true;
										     	$scope.remove_gettrans= false;
										     	$scope.timeouterror=err.message;
										     	$scope.amountcard="";
										     }
											     ...
											     {  $scope.amountcard="";
													...
													$scope.remove_gettrans= false;
										     		$scope.successfull_message=false;
										     		clearInterval(storeTimeInterval);
										     		$scope.timeouterror="No internet Connection Start the Transaction Again";
											     }
				   ...
				   $scope.loading_error = true;
				    ...
				    $scope.error_call=function() {
						     ...
						     {  $scope.amountcard="";
						     	...
						     	$scope.time_out_response=true;
						     	$scope.remove_gettrans= false;
						     	$scope.timeouterror=err.message+ 'Start the transaction again';
    ...
    $scope.signverify=function(tid,requestid) {
    	  var acceptence=true;
    	  $scope.httpRequest = dataService.signatureverification(tid,requestid,acceptence);
          $scope.sign_verify=false;
          
    }
     $scope.notverify=function(tid,requestid) {
     	   var acceptence=false;
    	  $scope.httpRequest = dataService.signatureverification(tid,requestid,acceptence);
          $scope.sign_verify=false;
          
    }

    $scope.canceltransaction=function(locurl) {
    	 $scope.httpRequest = dataService.cancelrequest(locurl);
    	 $scope.httpRequest.then(function(responsedata) {
    	   }).catch(function(err){
...
var added_nodefind={
 ...
 var countod_nodes=0;
 var details_node="";
 var doc_created=true;
 var count_node=0;
 var now_date = new Date().toISOString();
 var order_doc='AIVOOT_order_'+now_date;
 
	$scope.changeonlineaction=function(response)
	{
	var shopname=$rootScope.aiv_info.shop_name;
    var terminal_name=$scope.terminalData.name;
	var online_order_status=firebase.database().ref('Shops/'+shopname+'/'+terminal_name+'/onlineorderstatus');
    var getorders=firebase.database().ref('Shops/'+shopname+'/orders');
    var myConnectionsRef = firebase.database().ref('Shops/'+shopname+'/'+terminal_name+'/connections');
    var currentstatus = firebase.database().ref('Shops/'+shopname+'/'+terminal_name+'/status');
    var lastOnlineRef = firebase.database().ref('Shops/'+shopname+'/'+terminal_name+'/lastOnline');
    var connectedRef = firebase.database().ref(".info/connected");
		connectedRef.on("value", function(snap) {
	var now_date = new Date().toISOString();
		if (snap.val() === true) {
		  	$scope.posisactive=true;
		  	$scope.blinkconnectionloss=false;
		  	$scope.pos_connection_status.internet_connection=true;
		    $scope.pos_status = { 'online':true,'title':'online','class':'balanced','icon':'ion-more'};
		  	myConnectionsRef.onDisconnect().set(false);
		  	currentstatus.onDisconnect().set('Offline');
		  	lastOnlineRef.onDisconnect().set(firebase.database.ServerValue.TIMESTAMP);
		  	online_order_status.onDisconnect().set(false);
		  	myConnectionsRef.set(true);
		  	currentstatus.set('online');
		  	if(response){
		  		online_order_status.set(true);
				
		  	    getorders.on('value',gotUserData);
			  	  function gotUserData(snapshot){
			  		if($scope.onlineOrder.enable)
					{  
							
							 snapshot.forEach(userSnapshot => {
			  		 	var orders_key = userSnapshot.key;
	    				var order_value = userSnapshot.val();
					    var countof_firebase_node=snapshot.numChildren();
						countod_nodes++;
						console.log(orders_key);
							if(nodes.length==0)
							{ 
						
								added_nodefind.node=orders_key;
								added_nodefind.istrue=true;
						        nodes.push(added_nodefind);
							     //$scope.orderenode(); 
								
							}
							else{
								for(var const_node=0;countof_firebase_node>countod_nodes;const_node++)
								{
									if(!nodes[const_node].hasOwnProperty('istrue'))
									{  
										added_nodefind.node=orders_key;
										added_nodefind.istrue=true;
										nodes.push(added_nodefind);
							...
							console.log(nodes);
	    			  	    ...
	    			  	    /*$pouchDB.upsert(order_doc,function(doc){	
								      doc.data=order_value;
								  	return doc;
								  }).then(function(resData) {	
								 	 
								 	
								 		 var remove_order_node=firebase.database().ref('Shops/'+shopname+'/orders/'+orders_key);
								 		 remove_order_node.remove();
									    $pouchDB.get(order_doc).then(function(doc){
									    	console.log(order_doc);
												if(doc._id.startsWith('AIVOOT_')&&doc.data.status==="processing"){
									 			console.log(doc._id);
									 			if(angular.isDefined(doc.data.order_meta.processed_by) && doc.data.order_meta.processed_by=="AIVOOT"){
									 						add2Alert(doc.data.order_meta.bill_number);
																if($scope.online_order._id == undefined || $scope.POSSettings.online_default_accept==true){
										 							//Show new order
									 							if(angular.isDefined(doc.data.order_meta.delivery_time) && doc.data.order_meta.delivery_time!=""){
										 								if(doc.data.order_meta.delivery_time !='ASAP'){
																			doc.data.order_meta.delivery_time=new Date(doc.data.order_meta.delivery_time);
										 									doc.data.order_meta.del_hours =$filter('date')(doc.data.order_meta.delivery_time, "hh:mm a")
										 									doc.data.order_meta.duration="";
										 								}else{
										 									doc.data.order_meta.duration="ASAP";
									 									doc.data.order_meta.delivery_time=new Date();
									 									}
										 							}else{
																		if(doc.data.shipping_lines[0].method_id == 'local_delivery'){
																			doc.data.order_meta.duration = '40 Minutes';
																			doc =$scope.changeDurationInterval('40 Minutes',doc);
										 								}else if(doc.data.shipping_lines[0].method_id == 'local_pickup'){
										 									doc.data.order_meta.duration = '20 Minutes';
									 									doc =$scope.changeDurationInterval('20 Minutes',doc);
									 								}
									 							}
									 							var customer_info =shareData.getCustomer("PHONE",doc.data.customer.billing_address.phone);
										 							if(angular.isDefined(customer_info._id))
																		doc.data.customer.orders_count= customer_info.orders_count+1;
										 							$scope.online_order = angular.copy(doc);
																	//
																	$scope.showOnlineModal();
										 							//Play sound
			...
			{
			 online_order_status.set(false);
			}
		}
		else {
		    $scope.posisactive=false;
		    online_order_status.set(false);
		    $scope.blinkconnectionloss=true;
		    $scope.pos_status = { 'online':false,'title':'offline','class':'assertive blink','icon':'ion-more'};
		    $scope.pos_connection_status.internet_connection=false;
		    myConnectionsRef.set(false);
		    currentstatus.set('Offline');
	...
	$scope.orderenode=function()
   { 
   	       var shopname=$rootScope.aiv_info.shop_name;
           var terminal_name=$scope.terminalData.name;
			  		   
			  		    var get_array_nodelength=nodes.length;
						    
						    	if(doc_created==true&&get_array_nodelength>count_node)
						    	{
						    			 details_node=nodes[count_node];
						    			
						    			var getorders1=firebase.database().ref('Shops/'+shopname+'/orders/'+details_node);
										
						    	    	 getorders1.once('value').then(function(snapshot){
											
							    	 $pouchDB.upsert(order_doc,function(doc){
							    
							    	 	doc.data=snapshot.val();
									  	...
									  	var remove_order_node=firebase.database().ref('Shops/'+shopname+'/orders/'+details_node);
									 	remove_order_node.remove();
										nodes.splice(count_node).
										 count_node++;
									  	 $pouchDB.get(order_doc).then(function(doc){
									    	console.log(order_doc);
												if(doc._id.startsWith('AIVOOT_')&&doc.data.status==="processing"){
	...
	/*settings_forpaymentadded*/
	$scope.create_payment_options=function(create_payment_group)
	{
		
		var added_payment_name=$("#added_paymentname").val();
		if(create_payment_group!="nogroup")
		{
			if(create_payment_group=="card")
			{
				var newadded_payment_data={
				   "title": added_payment_name,
				   "description": "Pay with "+added_payment_name,
				   "method_title": added_payment_name,
				   "id": added_payment_name,
				   "fee":"0",
				   "enable": false
               };
			  ...
			  $pouchDB.upsert('getPaymentMethods',function(doc){
				if(doc){
					for(let pay_pos=0;pay_pos<$scope.settings_data.payment_methods.length;pay_pos++)
					{
						if($scope.settings_data.payment_methods[pay_pos]['id']=='card')
						{
							 $scope.settings_data.payment_methods[pay_pos]['payment_group'].push(newadded_payment_data);
							 console.log($scope.settings_data.payment_methods);
						}
					}
			  ...
			  console.log(error);
			  $scope.showCheckoutMsg('Error','An error occurred update payment methods!!!');
			  });
			...
			$pouchDB.upsert('getPaymentMethods',function(doc){
			if(doc){
				$scope.settings_data.payment_methods.push(newadded_payment_data);
				doc.methods = angular.copy($scope.settings_data.payment_methods);
				return doc;
			}
			return false;
