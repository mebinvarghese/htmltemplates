...
.factory('dataService', ['$http', '$q', 'APIFunctions', 'utility', '$filter','AIV_CONSTANTS','$rootScope', function($http, $q, APIFunctions, utility, $filter,AIV_CONSTANTS,$rootScope) {
	...
	var terminalurl='https://sd326x330000.test.connect.paymentsense.cloud/pac/terminals/';
	var username="";
    var password='abc90b10-3e2f-4280-89cd-0d7395862316';
    ...
    return {
        ...
        getpos_terminals: function() {
				...
				function( response ) {
					if(aborted){
						...
						return( $q.reject(response.data.messages.error[0]));
					}
				}
        ...
        posttransactionrequest: function(tid,amount) {
        	  alert(amount+'is');
      ...
      gettransactionresult: function(tid,requestedid) {
      ...
      signatureverification: function(tid,requestedid,accept) {
	  ...
	  firebaseorder: function(tid) {
				...
				if(deferred){
        		 ...
        		 var request = $pouchDB.upsert({timeout:deferred.promise});
